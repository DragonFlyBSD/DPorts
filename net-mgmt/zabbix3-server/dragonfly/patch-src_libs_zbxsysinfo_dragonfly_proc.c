...
+
 	for (i = 0; i < (int)(sz - 1); i++)
 		if (args[i] == '\0')
 			args[i] = ' ';
@@ -107,11 +98,7 @@ int     PROC_MEM(AGENT_REQUEST *request,
 	unsigned int	mibs;
...
@@ -213,11 +200,7 @@ int     PROC_MEM(AGENT_REQUEST *request,
 	}
 	else
 	{
-#if (__FreeBSD_version) < 500000
...
@@ -280,14 +263,7 @@ int     PROC_MEM(AGENT_REQUEST *request,
...
@@ -426,11 +402,7 @@ int	PROC_NUM(AGENT_REQUEST *request, AGE
 	}
 	else
 	{
-#if (__FreeBSD_version) > 500000
...
@@ -467,15 +439,15 @@ int	PROC_NUM(AGENT_REQUEST *request, AGE
 		{
