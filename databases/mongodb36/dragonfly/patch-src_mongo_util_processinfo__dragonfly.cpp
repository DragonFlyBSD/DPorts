--- src/mongo/util/processinfo_dragonfly.cpp.orig	2019-02-01 00:42:44 UTC
...
+    int vss = task->kp_vm_map_size / 1024 / 1024;  // convert from bytes to MB
+    kvm_close(kd);
+    return vss;
+}
+
...
+        return -1;
+    kinfo_proc* task = kvm_getprocs(kd, KERN_PROC_PID, _pid.toNative(), &cnt);
+    int rss = task->kp_vm_rssize * sysconf(_SC_PAGESIZE) / 1024 / 1024;  // convert from pages to MB
+    kvm_close(kd);
+    return rss;
+}
+
